...
- **Auditoria de Exclusividade da Versão na Janela:** Expandido o script de auditoria `audit_plugins_compliance.py` e o comando `tb_auto audit-plugins` para aplicar a checagem de exclusividade da versão (Regra 3), validando positivamente a configuração no `main.py` e proibindo com rigor a exibição de badges ou textos de versão redundantes no interior dos arquivos HTML (`ui/*.html`) dos plugins (#30).
